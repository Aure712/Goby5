#!/public/software/conda_envs/EDTA/bin/perl
##---------------------------------------------------------------------------##
##  File:
...
# ChangeLog
#
# 2023.11.11 by Xiaochi Zhou
#    Modified graphLabels and nameMap according to Wicker's classification standards
#    Also use /EDTA/util/TE_Sequence_Ontology.txt as reference
#
# 2025.11.27
#    - Force y-axis baseline at 0 for the stacked column chart.
#    - Collapse minor classes (Simple_repeat/Satellite/Structural_RNA/Low_complexity/Segmental)
#      into 'Other' and make both charts contain only the major categories.
###############################################################################
#
# To Do:
#
...
# Only keep major categories in both stacked plot and pie chart.
my $graphLabels = [
  [ 'Unknown',         '#999999' ],
  [ 'Other',           '#4D4D4D' ],
  [ 'DNA',             '#FF6A42' ],
...
# Build a helper map: label -> color
my %labelColor = map { $_->[0] => $_->[1] } @{$graphLabels};
...
# Warn on classes that won't be graphed (after normalization)
...
# Prepare label lists and color strings
my $colors               = "";
my $pieColors            = "slices: {";
my $idx                  = 0;
...
# Keep only major categories that have data
foreach my $label ( @{$graphLabels} ) {
...
# Colors for stacked columns in the same order as labels
if ( @landscapeGraphLabels ) {
  $colors = join(", ", map { "\"$labelColor{$_}\"" } @landscapeGraphLabels);
}

# Pie chart includes only the same major categories (no extras)
# Build per-slice colors aligned to the row order
my @genomeFractionChartLabels = @landscapeGraphLabels;
foreach my $label ( reverse @genomeFractionChartLabels ) {
  $pieColors .= "$idx: { color: \"$labelColor{$label}\" }, ";
  $idx++;
}
...
#
# Build per-divergence stacked values
#
my %pdata = ();
foreach my $div ( @div ) {
  next if $div > 50;
...
print <<_TAIL_
        var options = {
          ...
          vAxis: {title: 'percent of genome', viewWindowMode: 'explicit', viewWindow: {min: 0}, minValue: 0, baseline: 0},
          isStacked: 1,
          colors: [$colors]
        };
...
# Map many subclass names to normalized categories; collapse minor classes into 'Other'
sub fixName {
  my $r = shift;
  $r =~ s/\?//g;

  # Hard-coded map of class names to graph names (kept from prior version)
  my %nameMap = (
    "DNA/Chompy"           => "DNA",
    ...
    "DNA/T2"               => "DNA/Kolobok",
    "DNA/MULE-MuDR"        => "DNA/Mutator",
    "DNA/MULE-NOF"         => "DNA/Mutator",
    "DNA/MuDR"             => "DNA/Mutator",
    "DNA/DTM"              => "DNA/Mutator",
    "MITE/DTM"             => "DNA/Mutator",
    "DNA/piggyBac"         => "DNA/PiggyBac",
    "TIR/PiggyBac"         => "DNA/PiggyBac",
    "DNA/Sola-1"           => "DNA/Sola",
    "DNA/MER2_type"        => "DNA/Tc1-Mariner",
    "DNA/Mariner"          => "DNA/Tc1-Mariner",
    "DNA/Pogo"             => "DNA/Tc1-Mariner",
    "DNA/Stowaway"         => "DNA/Tc1-Mariner",
    "DNA/Tc1"              => "DNA/Tc1-Mariner",
    "DNA/Tc2"              => "DNA/Tc1-Mariner",
    "DNA/Tc4"              => "DNA/Tc1-Mariner",
    "DNA/TcMar"            => "DNA/Tc1-Mariner",
    "DNA/TcMar-Fot1"       => "DNA/Tc1-Mariner",
    "DNA/TcMar-ISRm11"     => "DNA/Tc1-Mariner",
    "DNA/TcMar-Mariner"    => "DNA/Tc1-Mariner",
    "DNA/TcMar-Pogo"       => "DNA/Tc1-Mariner",
    "DNA/TcMar-Tc1"        => "DNA/Tc1-Mariner",
    "DNA/TcMar-Tc2"        => "DNA/Tc1-Mariner",
    "DNA/TcMar-Tigger"     => "DNA/Tc1-Mariner",
    "DNA/Tigger"           => "DNA/Tc1-Mariner",
    "TIR/Tc1_Mariner"      => "DNA/Tc1-Mariner",
    ...
    "DNA/MULE-F"         => "DNA/Mutator",
    "DNA/TcMar-Stowaway" => "DNA/Tc1-Mariner",
    "DNA/TcMar-Tc4"      => "DNA/Tc1-Mariner",
    "DNA/TcMar-m44"      => "DNA/Tc1-Mariner",
    "DNA/DTT"            => "DNA/Tc1-Mariner",
    "MITE/DTT"           => "DNA/Tc1-Mariner",
    ...
    "Satellite/subtelomeric"=> "Satellite"
  );
  ...
  # Collapse subclasses to major categories and unify minor classes to 'Other'
  if ( $r =~ m{^(Simple_repeat|Satellite|Structural_RNA|Low_complexity)$} ) {
    $r = 'Other';
  }
  elsif ( $r =~ m{^(DNA|LINE|LTR|SINE)\b} ) {
    $r = $1;  # e.g. DNA/hAT -> DNA
  }
  elsif ( $r eq 'RC/Helitron' or $r eq 'Retroposon/SVA' or
          $r eq 'Unknown' or $r eq 'Other' ) {
    # keep as is
  }
  elsif ( $r eq 'Segmental' ) {
    $r = 'Other';   # merge Segmental duplications into Other
  }
  else {
    $r = 'Other';
  }
