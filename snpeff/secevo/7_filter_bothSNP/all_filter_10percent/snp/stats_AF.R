...
dat1 <- read.table("refTra.NONSYN.polarized.out.txt", header=F, sep="\t")
dat2 <- read.table("refTba.NONSYN.polarized.out.txt", header=F, sep="\t")

dat1_dedup <- dat1[!duplicated(dat1[, c(1,2)]), ]
dat2_dedup <- dat2[!duplicated(dat2[, c(1,2)]), ]

# 保存去重后的文件
write.table(dat1_dedup, "refTra.dedup.NONSYN.polarized.out.txt", 
            row.names=FALSE, col.names=FALSE, sep="\t", quote=FALSE)

write.table(dat2_dedup, "refTba.dedup.NONSYN.polarized.out.txt", 
            row.names=FALSE, col.names=FALSE, sep="\t", quote=FALSE)
...
# 保存为文本文件
write.table(dat1_dedup, file = "allpops.refTra.NONSYN.withAF.bed", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(dat2_dedup, file = "allpops.refTba.NONSYN.withAF.bed", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
