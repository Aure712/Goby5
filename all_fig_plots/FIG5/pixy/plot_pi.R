...
dxy_file    <- "Tra.refTra.pi.bed"  # 实际文件名
...
analyze_and_plot <- function(dat, tag) {
  ...
  stats_file <- sprintf("pi_tra_normal_vs_invert_stats_%s.tsv", tag)
  ...
  g <- ggplot(dat, aes(x = type, y = dxy, fill = type)) +
    ...
    ylab("pi") +
    ggtitle(sprintf("Pi distribution in Tra Normal vs Tra Inversion (%s)", tag))
  
  plot_file <- sprintf("Pi_tra_normal_vs_invert_violin_%s.png", tag)
