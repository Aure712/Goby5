...
dxy_file    <- "Tba.Tra.fst.bed"  # 实际文件名
...
analyze_and_plot <- function(dat, tag) {
  ...
  stats_file <- sprintf("Fst_normal_vs_invert_stats_%s.tsv", tag)
  ...
  g <- ggplot(dat, aes(x = type, y = dxy, fill = type)) +
    ...
    ylab("fst") +
    ggtitle(sprintf("Fst distribution in Normal vs Inversion (%s)", tag))
  
  plot_file <- sprintf("Fst_tba_normal_vs_invert_violin_%s.png", tag)
